...
GraphWidget gr =
    GraphWidget(&tft);  // Graph widget gr instance with pointer to tft
...
void setup() {
  ...
  tr.addPoint(0.0, 0.0);
  // Add another point at 100.0, 0.0 this will be joined via line to the last
  // point added
...
void loop() {
  ...
  if (millis() - plotTime >= 100) {
    ...
    // Add a plot, first point in a trace will be a single pixel (if within
    // graph area)
    tr.addPoint(gx, gy);
    gx += 1.0;
